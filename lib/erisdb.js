...
var nameregF = require('./nameRegistry');
 ...
 */
exports.createInstance = function (client, validator, privateKey) {
    return new ErisDB(client, validator, privateKey);
};
 ...
 * @constructor
 */
function ErisDB(client, validator, privateKey) {
    ...
    var namereg = nameregF.createInstance(client, unsafe, events, privateKey);
...
/**
 * Closes the connection. This method calls the <tt>shutDown</tt> method on the client.
 */
ErisDB.prototype.close = function () {
    if (!this._client) {
        throw new Error("Networking client object has not been set.");
    }
    return this._client.shutDownAsync();
};
